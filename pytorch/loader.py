...
def load_mat(filepath):
    ...
    drug_sim = mat["drug"].astype(np.cfloat)
    disease_sim = mat["disease"].astype(np.cfloat)
...
def load_lrssl(filepath, reduce=True):
    ...
    drug_sim = rr.astype(np.cfloat)
    disease_sim = dd.astype(np.cfloat)
        ...
        if type == "train":
            ...
            for i in range(num_batch):
                ...
                self.batch_data.append(
                    [torch.FloatTensor(disease_drug_Adj),
                     torch.FloatTensor(disease_disease_sim_Matrix),
                     torch.FloatTensor(drug_drug_sim_Matrix),
                     torch.LongTensor(input_disease),
                     torch.LongTensor(input_drug),
