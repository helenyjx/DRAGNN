...
def cross_validation():
    ...
    for fold_num in range(len(all_train_mask)):
        ...
        for epoch in range(config['max_epoch']):
            ...
            auroc, aupr = evaluate(model, "valid", valid_manager, logger, fold_num)
            print("fold {} valid auroc :{:>.5f}".format(fold_num + 1, auroc))
            print("fold {} valid aupr :{:>.5f}".format(fold_num + 1, aupr))
        final_test_auroc, final_test_aupr = evaluate(model, "test", test_manager, logger, fold_num)
...
def evaluate(model, name, data, logger, fold_num=0):
